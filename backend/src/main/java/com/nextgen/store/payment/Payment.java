...
public class Payment {
  ...
  public void setGateway(String value){gateway=value;} public void setGatewayOrderId(String value){gatewayOrderId=value;} public void setGatewayPaymentId(String value){gatewayPaymentId=value;}
  public void transitionTo(PaymentStatus next){
    if(next==null||next==status)return;
    boolean valid=switch(status){
      case PENDING -> next==PaymentStatus.AUTHORIZED||next==PaymentStatus.CAPTURED||next==PaymentStatus.FAILED;
      case AUTHORIZED -> next==PaymentStatus.CAPTURED||next==PaymentStatus.FAILED;
      case CAPTURED -> next==PaymentStatus.REFUNDED;
      case FAILED,REFUNDED -> false;
    };
    if(!valid)throw new IllegalStateException("Invalid payment status transition: "+status+" -> "+next);
    status=next;
  }
}
