...
public class RazorpayPaymentController {
  ...
  public RazorpayPaymentController(PaymentRepository payments, OrderRepository orders, UserRepository users,
      @Value("${razorpay.key-id:}") String keyId, @Value("${razorpay.key-secret:}") String keySecret) {
    this.payments = payments; this.orders = orders; this.users = users; this.keyId = keyId; this.keySecret = keySecret;
  }
  ...
  public CreateResponse createOrder(Authentication auth, @RequestBody Request request) throws Exception {
    ...
    options.put("amount", localOrder.getTotalAmount().movePointRight(2).longValueExact());
    options.put("currency", localOrder.getCurrency());
    options.put("receipt", "GENZ-" + localOrder.getId());
    Order razorpayOrder = client.orders.create(options);
    payment.setGateway("RAZORPAY"); payment.setGatewayOrderId(razorpayOrder.get("id")); payments.save(payment);
  ...
  public VerifyResponse verify(Authentication auth, @RequestBody VerifyRequest request) throws Exception {
    var user = users.findByEmailIgnoreCase(auth.getName()).orElseThrow(() -> new NoSuchElementException("User not found"));
    var localOrder = orders.findByIdAndUserId(request.orderId(), user.getId()).orElseThrow(() -> new NoSuchElementException("Order not found"));
    Payment payment = payments.findByOrderId(localOrder.getId()).orElseThrow(() -> new NoSuchElementException("Payment not found"));
    ...
    payment.setGatewayPaymentId(request.razorpayPaymentId()); payment.setStatus(PaymentStatus.CAPTURED); payments.save(payment);
    return new VerifyResponse(payment.getId(), localOrder.getId(), payment.getStatus().name());
  }

  public record Request(Long orderId) {}
  public record CreateResponse(Long paymentId, Long orderId, String keyId, String razorpayOrderId, String amount, String currency) {}
  public record VerifyRequest(Long orderId, String razorpayOrderId, String razorpayPaymentId, String razorpaySignature) {}
