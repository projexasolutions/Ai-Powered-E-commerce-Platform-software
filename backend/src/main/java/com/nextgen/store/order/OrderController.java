...
public class OrderController {
  ...
  public OrderResponse create(Authentication auth,@Valid @RequestBody CreateOrder request){
    ...
    Order order=orders.save(new Order(u,address,subtotal,shipping,subtotal.add(shipping)));
  ...
  public record CreateOrder(Long addressId,List<Item> items){}
  ...
  public record OrderResponse(Long id,String status,BigDecimal subtotal,BigDecimal shippingAmount,BigDecimal totalAmount,String currency,List<OrderItemResponse> items){static OrderResponse from(Order o){return new OrderResponse(o.getId(),o.getStatus().name(),o.getSubtotal(),o.getShippingAmount(),o.getTotalAmount(),o.getCurrency(),o.getItems().stream().map(i->new OrderItemResponse(i.getProductName(),i.getSku(),i.getSize(),i.getColor(),i.getQuantity(),i.getUnitPrice(),i.getLineTotal())).toList());}}
}
