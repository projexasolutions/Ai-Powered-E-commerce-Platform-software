 ...
 return <section className="detailPage">
   ...
   <div className="detailGrid">
     ...
     <div className="detailInfo">
       ...
       <h1>{detail.name}</h1>
       <div className="detailRating"><Star size={13} fill="currentColor"/> {detail.rating} <span>·</span> Loved by the GenZ community</div>
       <div className="detailPrice">{money(selected?.price??detail.price)}</div>
       <p className="detailDescription">{detail.description}</p>
       <div className="detailRule"/>
       <div className="optionBlock"><div className="optionHead"><strong>Size</strong><span>{size||'Select a size'}</span></div><div className="optionRow">{sizes.map(s=>{const available=detail.variants.some(v=>v.size===s&&v.stockQuantity>0);return <button key={s} disabled={!available} className={size===s?'active':''} onClick={()=>{setSize(s);const next=detail.variants.find(v=>v.size===s&&v.stockQuantity>0);if(next)setColor(next.color)}}>{s}</button>})}</div></div>
       <div className="optionBlock"><div className="optionHead"><strong>Color</strong><span>{color||'Select a color'}</span></div><div className="colorRow">{colors.map(c=>{const available=detail.variants.some(v=>v.color===c&&v.stockQuantity>0);return <button key={c} disabled={!available} className={color===c?'active':''} onClick={()=>{setColor(c);const next=detail.variants.find(v=>v.color===c&&v.stockQuantity>0);if(next)setSize(next.size)}}><i/>{c}</button>})}</div></div>
       <div className="detailPurchase"><div className="detailQty"><button onClick={()=>setQuantity(q=>Math.max(1,q-1))}><Minus/></button><span>{quantity}</span><button onClick={()=>setQuantity(q=>Math.min(selected?.stockQuantity||1,q+1))}><Plus/></button></div><button className="detailAdd" onClick={add}><ShoppingBag/> Add to bag <ArrowRight/></button></div>
       <div className="detailStatus">{selected?<><span className={selected.stockQuantity<=5?'lowStock':''}>{selected.stockQuantity<=5?`Only ${selected.stockQuantity} left`:'In stock'}</span><small>SKU {selected.sku}</small></>:<span>Select options to check availability</span>}</div>
       {error&&<div className={`detailNotice ${error.startsWith('Added')?'success':''}`}>{error}</div>}
       <div className="detailPromises"><div><Truck/><span><strong>Free shipping</strong>On orders above ₹999</span></div><div><ShieldCheck/><span><strong>Secure checkout</strong>Protected payments</span></div></div>
     </div>
   </div>
